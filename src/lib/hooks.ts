...
export function useJobItem(id: number | null) {
  ...
  return { jobItem: data?.jobItem } as const;
}
...
export function useJobItems(ids: number[]) {
  ...
  const jobItems = results
    ...
    // sometimes typescript doesn't do the filter correctly - thinks it is still undefined
    // .filter((jobItem) => jobItem !== undefined);
    //only actual jobItem object will be added to the filter array
    .filter((jobItem) => Boolean(jobItem)) as JobItemExpanded[];
  //some results may still be loading...
...
export function useSearchQuery(searchText: string) {
  ...
  return { jobItems: data?.jobItems } as const;
}
...
export function useLocalStorage<T>(
  ...
  // useState initializer function. Only runs once.
